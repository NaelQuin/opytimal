...
meshPath = './InputData/Meshes/2D/rectangle16'
boundaryDataPath = './InputData/Meshes/2D/rectangle16_mf'
...
def inInlet(x):
    return x in map(tuple, boundarySubMeshes['inlet'].coordinates())

def inOutlet(x):
    return x in map(tuple, boundarySubMeshes['outlet'].coordinates())

def inWall(x):
    return x in map(tuple, boundarySubMeshes['wall'].coordinates())
...
L = zn*v*dx\
    ...
    - dt*(1-teta)*dot(grad(zn), grad(v))*dx
